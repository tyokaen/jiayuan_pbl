...
void cbToCoNet_vRxEvent(tsRxDataApp *pRx) {
	...
	switch (mode) {
		case 'X':
			vfPrintf(&sSerStream, LB"[PKT Ad:%04x, %d]",
				pRx->u32SrcAddr,
		...
		case 'D':
			//vfPrintf(&sSerStream, LB"[PKT Ad:%04x, ");
			for (j=0 ; j<30; j++) {
				acceData[j] = G_BE_WORD();
				//vfPrintf(&sSerStream, "%d, ", acceData[j]);
			}
			/*vfPrintf(&sSerStream, LB"[PKT Ad:%04x, id:%d, %d, %d, %d, %d]",
					pRx->u32SrcAddr,
					//pRx->u8Len+4, // Actual payload byte: the network layer uses additional 4 bytes.
					//pRx->u8Seq,
					//pRx->u8Lqi,
			...
			id += 10;
			//vfPrintf(&sSerStream, "]");
			vfPrintf(&sSerStream, LB"[PKT Ad:%04x, id:%d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d]",
				pRx->u32SrcAddr,
				//pRx->u8Len+4, // Actual payload byte: the network layer uses additional 4 bytes.
				//pRx->u8Seq,
				//pRx->u8Lqi,
